...
כדי לעקוב אחרי פרויקט **נוסף** (במקביל — כל פרויקט מקבל טאב משלו בווידג'ט), העתק את בלוק ה-`hooks`
ל-`.claude/settings.json` של אותו פרויקט (הנתיב לסקריפט הוא אבסולוטי, כך שאין צורך להעתיק קבצים).
אם לפרויקט כבר יש hooks אחרים רשומים — מוסיפים את בלוק ה-`send-event.js` לצד הקיימים באותו מערך
(ראו דוגמה ב-PensiaMng: graphify hook-guard + hooks של הווידג'ט חיים יחד תחת `PreToolUse`).

לאחר עריכת settings.json יש להפעיל מחדש את session של Claude Code כדי שה-hooks ייטענו.
